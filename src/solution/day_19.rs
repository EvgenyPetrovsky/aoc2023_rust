...
impl DaySolution {
    fn parse_one_rule(line: &str) -> Rule {
        ...
        let condition = re_condition.captures(line).map(|c| match c.get(0) {
            Some(_) => {
        ...
        let decision = re_decision.captures(line).map(|c| match c.get(0) {
            Some(_) => {
                let (_, [dec]) = c.extract();
    ...
    fn classify_one_part(
        ...
        let opt_decision: Option<Decision> = rules.iter().fold(None, |z, rule| {
            let new_decision = match rule {
                Rule::Condition(c) => {
                    let lhs = match c.par {
                        'x' => part.x,
                        'm' => part.m,
    ...
    fn find_all_combinations(
        ...
        let combinations: usize = rules
            ...
            .fold(
                (Some(part), 0_usize),
                |(opt_part, acc_combinations), rule| {
                    match opt_part {
                        Some(part) => match rule {
                            Rule::Condition(c) => {
                                let lhs = match c.par {
                                    'x' => part.x,
                                    'm' => part.m,
                                    ...
                                    's' => part.s,
                                    _ => unreachable!(
                                        "pasrt name is not in XMAS list (condition:{:?})",
                                        &c
                                    ),
                                };
                                match c.cmp {
                                    ...
                                    Comparison::Gt => {
                                        ...
                                        // range partially meets condition: split and process according to decision
                                            ...
                                            let (p1, p2) =
                                                Self::split_part_ranges(part, c.par, c.val + 1);
                                            let new_combinations =
                                                Self::find_all_combinations(&p1, flow, flows)
                                                    + Self::decision_to_combinations(
                                                        &c.dec, &p2, flows,
                                                    );
                                            (None, acc_combinations + new_combinations)
                                    ...
                                    Comparison::Lt => {
                                        ...
                                        // range partially meets condition: split and process according to decision
                                            ...
                                            let (p1, p2) =
                                                Self::split_part_ranges(part, c.par, c.val);
                                            let new_combinations =
                            ...
                            Rule::Decision(d) => {
                                let new_combinations =
                                    Self::decision_to_combinations(d, part, flows);
                                (None, acc_combinations + new_combinations)
                            }
                        },
...
impl super::Solution for DaySolution {
    ...
    fn solve_part_1(problem: Self::Problem) -> Self::Answer {
        ...
        let answer = parts
            .iter()
            .filter(
                |part| match DaySolution::classify_one_part(part, &start, &flows) {
